  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-400 dark:border-gray-700 p-4 sm:p-6 w-full">
      <div className="flex items-center gap-2 mb-4 sm:mb-6">
        ...
        <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900 dark:text-white">Department Capacity</h2>
      </div>
        ...
        {departments.map((dept, idx) => (
          <div key={idx} className="w-full">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 sm:gap-2 mb-2">
              <h3 className="text-xs sm:text-sm font-semibold text-gray-900 dark:text-white truncate">{dept.name}</h3>
              <span className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 whitespace-nowrap">
                {dept.waiting} waiting <span className="mx-1 sm:mx-2">•</span> {dept.current} / {dept.max}
              </span>
            </div>
            <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 sm:h-2 overflow-hidden">
              <div
                className="bg-blue-600 dark:bg-blue-500 h-full rounded-full transition-all duration-300"
                style={{ width: `${getProgressPercentage(dept.current, dept.max)}%` }}
              ></div>
            </div>
          </div>
        ))}
